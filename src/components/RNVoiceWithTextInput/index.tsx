...
const RNVoiceWithTextInput = ({ onNextPress, customStyle  }: VoiceQuestionProps) => {
    ...
    return (
        <>
            <View style={[styles.container , (customStyle && customStyle)]} >
                ...
                <RNTextComponent style={[styles.heading,  {color : themeColor.black , opacity : 0.6}]} isSemiBold>
                    Can you remember what{`\n`}
                    </RNTextComponent>
                    <RNTextComponent isSemiBold style={[styles.heading , {marginTop :0} ]} >
                        the cat was called?
                </RNTextComponent>
                <RNTextInputWithLabel inputStyle={styles.input} containerStyle={{marginTop  :verticalScale(21)}} />
                <Pressable onPress={toggleMic} style={styles.icon} >
                {micStatus ? <MicOn /> : <Mic />}

            </Pressable>
            </View>
            <RNButton customStyle={styles.footerButton} title={en.ANSWER} onClick={onNextPress} textStyle={{ fontSize: verticalScale(16) }} />
        </>
    )
}
