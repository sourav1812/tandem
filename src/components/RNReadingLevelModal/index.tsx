      ...
      />
            <View style={[styles.container , (isTablet && {width : verticalScale(270) , alignSelf : 'center'} ) ]}>
                ...
                <View style={styles.content} >
                    ...
                    <View style={[styles.content, { marginTop: 0, alignItems: 'flex-end', marginHorizontal: verticalScale(18) }]} >
                        {indicators.map((item, index) => <View style={[styles.indicator, { height: verticalScale(12) + verticalScale(5.5) * index, backgroundColor: item.color }]} />)}
                    </View>
                    <RNButton onlyIcon icon={<More />} onClick={() => { }} />
                </View>
            <RNButton title='Update' customStyle={styles.button} onClick={()=>{}}/>
            </View>
        </Modal>
    );
};
